"""Summarize nationwide US county vaccination MPLE experiment folders."""
...
from common import EXPERIMENT_ROOT, INTERVENTION_SPECS, OUTCOME_SPECS, parse_experiment_name  # noqa: E402
...
def collect_rows(experiments_root: Path) -> list[dict[str, object]]:
    ...
    for summary_path in summary_paths:
        ...
        row: dict[str, object] = {
            ...
            "outcome_label": OUTCOME_SPECS[parsed["outcome_code"]].label,
            "intervention_code": parsed["intervention_code"],
            "intervention_label": INTERVENTION_SPECS[parsed["intervention_code"]].label,
...
def write_csv(path: Path, rows: list[dict[str, object]]) -> None:
    if not rows:
        raise ValueError("No finished US county vaccination experiments were found.")
    fieldnames: list[str] = []
    seen: set[str] = set()
    for row in rows:
        for key in row.keys():
...
def write_markdown(path: Path, rows: list[dict[str, object]]) -> None:
    ...
    with path.open("w", encoding="utf-8") as handle:
        handle.write("# US County Vaccination MPLE Experiment Summary\n\n")
        handle.write(
            "This table summarizes every finished nationwide US county vaccination experiment folder "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Summarize nationwide US county vaccination MPLE experiment folders."
    )
    parser.add_argument(
        "--experiments_root",
        type=Path,
        default=EXPERIMENT_ROOT,
        help="Root directory containing US county vaccination experiment folders.",
    )
