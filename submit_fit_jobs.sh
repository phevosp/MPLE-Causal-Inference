...
PY
  ...
  while IFS="${FIELD_SEP}" read -r generation_manifest_path fits_spec_path scope_slug split_kind num_folds outer_num_folds test_fold_id experiment_name experiment_slug variant_name variant_slug fit_path; do
    worker_args=(
      --parsable
      --job-name "${WORKER_JOB_NAME}"
      --time "${FIT_TIME}"
    ...
    submit_output="$(
      ...
      TEST_FOLD_ID="${test_fold_id}" \
      FIT_OVERWRITE="${FIT_OVERWRITE}" \
      "${SBATCH_BIN}" "${worker_args[@]}" "${WORKER_SCRIPT}" "${experiment_slug}" "${variant_slug}" "${scope_slug}"
    )"
    job_ids+=("${submit_output%%;*}")
  done < <(
    pixi run python - <<'PY' "${GENERATION_MANIFEST_PATH}" "${FITS_SPEC_PATH}" "${SCOPE_SLUG}" "${SPLIT_KIND}" "${NUM_FOLDS}" "${OUTER_NUM_FOLDS}" "${TEST_FOLD_ID}"
import sys
from run_fit_pipeline import collect_direct_train_fit_request_rows

manifest_path, fits_spec_path, scope_slug, split_kind, num_folds, outer_num_folds, test_fold_id = sys.argv[1:]
kwargs = {}
if split_kind.strip():
    kwargs["split_kind"] = split_kind.strip()
if num_folds.strip():
...
for row in collect_direct_train_fit_request_rows(
    manifest_path,
    fits_spec_path,
    scope_slug.strip(),
    **kwargs,
...
if [[ "${FIT_MODE}" == "standard" ]]; then
  ...
  refresh_cmd="pixi run python -u run_fit_pipeline.py --fit_mode masked_standard --manifest_path '${GENERATION_MANIFEST_PATH}' --fits_spec_path '${FITS_SPEC_PATH}' --scope_slug '${SCOPE_SLUG}'"
  if [[ -n "${SPLIT_KIND}" ]]; then
    refresh_cmd="${refresh_cmd} --split_kind '${SPLIT_KIND}'"
  fi
  if [[ -n "${NUM_FOLDS}" ]]; then
