...
def _run_masked_fit(
    ...
    *,
    ...
    resolved_extra_metadata: dict[str, object] = {
        "search_slug": str(search_slug),
    ...
    if str(split_artifacts["split_kind"]) == "test_train_cv":
        resolved_extra_metadata["outer_num_folds"] = int(outer_num_folds)
        resolved_extra_metadata["test_fold_id"] = int(test_fold_id)
    if extra_metadata:
        resolved_extra_metadata.update(extra_metadata)
    ...
    materialize_fit_root(
        ...
        extra_metadata=resolved_extra_metadata,
    )
    ...
    *,
    ...
    return _run_masked_fit(
        ...
        extra_metadata={
            ...
            "best_candidate_path": path_text(best_candidate_path),
            "cv_spec_path": ""
            if cv_spec_path in (None, "")
            else path_text(cv_spec_path),
        },
        overwrite=overwrite,
    )
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        elif fit_mode == FIT_MODE_OUTER_MASKED:
